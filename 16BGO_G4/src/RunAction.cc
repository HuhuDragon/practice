...
namespace B1
{
  ...
  RunAction::RunAction() : G4UserRunAction(), fNCounts(0), fPriE(0), fPriE1(0), fPriName(0), fPriName1(0), fout(NULL), ftr(NULL), fTimer(NULL)
  {
    fNCounts = 0;
  ...
  void RunAction::BeginOfRunAction(const G4Run *run)
  {
    ...
    if (IsMaster())
    {
      ...
      G4cout << "-----Start of Work Run-----" << G4endl;
    if (IsMaster())
    {
      fout = new TFile("out.root", "RECREATE");
      ftr = new TTree("tr", "tr");
      ftr->Branch("de", fEdep, "de[16]/D");
      ftr->Branch("Ts", fTs, "Ts[16]/D");
      ...
      ftr->Branch("timestamp", &fRunTimeStampe, "timestamp/D"); // 添加时间戳的branch
      ftr->Branch("tTs", ftTs, "tTs[16]/D");                    // 添加时间戳的branch
    }
    // fTimeStampe = 0.;
  }
  ...
  void RunAction::SetAndFill(G4double fE[nScoringVolumes], G4double fT[nScoringVolumes], G4double pE, G4double pE1, G4int pID, G4int pID1)
  {
    ...
    for (int i = 0; i < nScoringVolumes; i++)
      ((RunAction *)G4MTRunManager::GetMasterRunManager()->GetUserRunAction())->fEdep[i] = fE[i];
    for (int i = 0; i < nScoringVolumes; i++)
      ((RunAction *)G4MTRunManager::GetMasterRunManager()->GetUserRunAction())->fTs[i] = fT[i];
    ...
    // G4cout << "fTimeStampe_RUNACTION:" << fTimeStampe << G4endl;
    ((RunAction *)G4MTRunManager::GetMasterRunManager()->GetUserRunAction())->fRunTimeStampe = fTimeStampe / ns;
    for (int i = 0; i < nScoringVolumes; i++)
      if (fTs[i] > 0)
        ((RunAction *)G4MTRunManager::GetMasterRunManager()->GetUserRunAction())->ftTs[i] = fT[i] + fTimeStampe / ns;
    ((RunAction *)G4MTRunManager::GetMasterRunManager()->GetUserRunAction())->ftr->Fill();
  }
  //....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
  void RunAction::SetTimeStamp(G4double eventTimeStamp)
  {
    fTimeStampe = eventTimeStamp;
  }
}
