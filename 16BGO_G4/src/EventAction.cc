...
namespace B1
{
  ...
  void EventAction::BeginOfEventAction(const G4Event *event)
  {
    G4int beamNT = 3;
    G4int measuredNT = 8;
    G4double Thalf = 122.24e9 * ns;
    G4double period = Thalf * (beamNT + measuredNT) * ns; // 周期 = 122*8 ns
    G4double sampleWindow = Thalf * beamNT * ns;          // 采样窗口 = 122*3 ns

    // 在当前周期的采样窗口中随机选择时间戳
    G4double randomInWindow = G4UniformRand() * sampleWindow;

    // 将粒子枪随机分配到若干周期
    G4int BeamOn = 2;
    G4int cycleIndex = static_cast<G4int>(G4UniformRand() * BeamOn);

    fTimestamp = randomInWindow + cycleIndex * period;
    fRunAction->SetTimeStamp(fTimestamp);
    // G4cout << "fTimestamp_EVENT:" << fTimestamp / s << G4endl;
  ...
  void EventAction::EndOfEventAction(const G4Event *)
  {
    ...
    if (priname == "mu-" || priname == "mu+")
      fPriName = 1;
    else if (priname == "gamma")
      fPriName = 2;
    else if (priname == "e-" || priname == "e+")
      fPriName = 3;
    else if (priname == "proton")
      fPriName = 4;
    else if (priname == "neutron")
      fPriName = 5;
    TString priname1 = particleGun1->GetParticleDefinition()->GetParticleName();
    if (priname1 == "mu-" || priname1 == "mu+")
      fPriName1 = 1;
    else if (priname1 == "gamma")
      fPriName1 = 2;
    else if (priname1 == "e-" || priname1 == "e+")
      fPriName1 = 3;
    else if (priname1 == "proton")
      fPriName1 = 4;
    else if (priname1 == "neutron")
      fPriName1 = 5;
