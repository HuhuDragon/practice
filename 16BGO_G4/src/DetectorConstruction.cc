...
namespace B1
{
  ...
  G4VPhysicalVolume *DetectorConstruction::Construct()
  {
    ...
    G4Material *mCr = nist->FindOrBuildMaterial("G4_Cr");
    ...
    mPE_B->AddMaterial(mB, PE_Bfractionmass = 20 * perCent);
    ...
    auto CTVisAtt = new G4VisAttributes(G4Colour(0.45, 0.25, 0.0, 0.5));
    ...
    G4double holeSpacing = 0.1 * mm;                // 孔的间距图纸4.85
    ...
    G4LogicalVolume *logicPE_B = new G4LogicalVolume(PE_Bsub, mAir, "PE_B"); // 将含硼聚乙烯改成空气成分
...
#if 1
    // // target//靶
    G4double sizeTarThickness = 1.0 * mm;
    G4double sizeTarRadians = 15. / 2 * mm;
    ...
    G4LogicalVolume *logicTar = new G4LogicalVolume(solidTar, mCr, "Tar"); // 注意衬底材料
    ...
    G4double sizeVaTRouter = 30 * mm;  // 靶管外径61mm；
    G4double sizeVaTRinner = 27. * mm; // 旧模拟使用内径48mm；图纸内径54cm；
    G4ThreeVector posVaTVector = G4ThreeVector(0., 0., sizeVaTThickness / 2. - 1. * mm + shiftd);
    ...
    // beam tube2
    G4double sizeVaT2Thickness = 5.0 * mm;
    G4double sizeVaT2Router = 54. / 2. * mm;
    G4double sizeVaT2Rinner = 15 / 2. * mm;
    G4ThreeVector posVaT2Vector = G4ThreeVector(0., 0., sizeVaT2Thickness / 2 - 1. * mm + shiftd);
    ...
    // beam tube3
    G4double sizeVaT3Thickness = 4. * mm;
    G4double sizeVaT3Router = 15 / 2. * mm;
    G4double sizeVaT3Rinner = 10 / 2. * mm;
    ...
    //-300V
    G4double Fu300Z = 1. * mm;
    G4double Fu300OuterR = 38 / 2. * mm;
    G4double Fu300InnerR = 10 / 2. * mm;

    G4Tubs *solidTFu300 = new G4Tubs("Fu300", Fu300InnerR, Fu300OuterR, Fu300Z * 0.5, 0. * deg, 360. * deg);
    G4ThreeVector posFu300Vector = G4ThreeVector(0., 0., shiftd + CTdistance + Fu300Z / 2);
    G4LogicalVolume *logicFu300 = new G4LogicalVolume(solidTFu300, mCu, "Fu300");
    new G4PVPlacement(0, posFu300Vector, logicFu300, "Fu300", logicEnv, false, 0, checkOverlaps);
    logicFu300->SetVisAttributes(CTVisAtt);

    // Ta准直孔
    G4double TacollZ = 1. * mm;
    G4double TacollOuterR = 38 / 2. * mm;
    G4double TacollInnerR = 5 / 2. * mm;
    G4double Tacolldis = 17 * mm;

    G4Tubs *solidTacoll = new G4Tubs("Tacoll", TacollInnerR, TacollOuterR, TacollZ * 0.5, 0. * deg, 360. * deg);
    G4ThreeVector posTacollVector = G4ThreeVector(0., 0., shiftd + CTdistance + Fu300Z / 2 + Tacolldis);
    G4LogicalVolume *logicTacoll = new G4LogicalVolume(solidTacoll, mTa, "Tacoll");
    new G4PVPlacement(0, posTacollVector, logicTacoll, "Tacoll", logicEnv, false, 0, checkOverlaps);
    logicTacoll->SetVisAttributes(CTVisAtt);
    ...
    // 新靶后盖
    G4double shape1Z = 3. * mm;
    G4double shape1R = 40 / 2. * mm;

    G4Tubs *solidTarback = new G4Tubs("Tarback", 0.0 * mm, shape1R, shape1Z * 0.5, 0. * deg, 360. * deg);
    G4ThreeVector posTarbackVector = G4ThreeVector(0., 0., -sizeTarThickness - shape1Z / 2 + shiftd);
    G4LogicalVolume *logicTarback = new G4LogicalVolume(solidTarback, mPla, "Tarback");
    new G4PVPlacement(0, posTarbackVector, logicTarback, "Tarback", logicEnv, false, 0, checkOverlaps);
    logicTarback->SetVisAttributes(TaVisAtt);
#endif
    /**********************************Pb shadows are here*************************************/
#if 0
    // Pb_Bottom
